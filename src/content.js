export const content = {
  hero: {
    title: "DÂN CHỦ XHCN",
    subtitle: "Giữa Lý luận Kinh điển và Sức mạnh Không gian mạng",
    typing: [
      "Mạng xã hội mở rộng phương thức dân chủ...",
      "Nhưng công cụ không thay thế bản chất chế độ...",
      "Quyền lực nhân dân phải được bảo đảm bằng thể chế và pháp luật...",
    ],
  },

  part1: {
    title: "Nhận định và phân tích tình huống",
    intro:
      "Nhận định ban đầu đúng một phần vì nêu được vai trò mới của mạng xã hội, nhưng chưa chính xác khi đồng nhất tự do biểu đạt trên mạng với biểu hiện cao nhất của dân chủ xã hội chủ nghĩa.",
    correct: {
      title: "Phần đúng của nhận định",
      points: [
        "Mạng xã hội giúp lôi cuốn quần chúng tham gia quản lý xã hội nhanh và rộng hơn.",
        "Không gian mạng hỗ trợ thực hiện quyền tự do ngôn luận và giám sát xã hội.",
        "Áp lực dư luận tích cực thúc đẩy chính quyền phản hồi và tăng trách nhiệm giải trình.",
      ],
    },
    incorrect: {
      title: "Phần sai của nhận định",
      points: [
        "Nhầm lẫn giữa công cụ kỹ thuật và bản chất của chế độ chính trị.",
        "Thiếu góc nhìn giai cấp và vai trò lãnh đạo của Đảng Cộng sản.",
        "Không phản ánh cơ sở kinh tế của quyền làm chủ là sở hữu xã hội về tư liệu sản xuất chủ yếu.",
        "Nội dung viral không có giá trị pháp lý thay thế cơ chế bầu cử và quyết nghị theo luật.",
      ],
    },
  },

  part2: {
    title: "Căn cứ lý luận - bản chất dân chủ XHCN",
    political: {
      title: "Bản chất chính trị",
      points: [
        "Dân chủ XHCN là quyền lực của đại đa số nhân dân lao động.",
        "Đặt dưới sự lãnh đạo của Đảng để bảo đảm định hướng vì lợi ích chung.",
        "Mục tiêu là lôi cuốn quần chúng tham gia tự giác vào quản lý nhà nước và xã hội.",
      ],
    },
    economic: {
      title: "Bản chất kinh tế",
      points: [
        "Nền tảng là chế độ sở hữu xã hội về tư liệu sản xuất chủ yếu.",
        "Quyền làm chủ kinh tế là cơ sở bảo đảm quyền làm chủ chính trị thực chất.",
        "Lợi ích của người lao động là động lực phát triển kinh tế - xã hội.",
      ],
    },
    ideology: {
      title: "Bản chất tư tưởng - văn hóa",
      points: [
        "Lấy chủ nghĩa Mác - Lênin làm nền tảng tư tưởng cốt lõi.",
        "Kế thừa tinh hoa văn hóa nhân loại và phát huy sức sáng tạo của cá nhân.",
        "Kết hợp hài hòa lợi ích cá nhân, tập thể và toàn xã hội.",
      ],
    },
    criteria: {
      title: "Tiêu chí quyền lực thực sự thuộc về nhân dân",
      points: [
        "Nhân dân là chủ thể quyền lực: dân là chủ và dân làm chủ.",
        "Quyền lực được thực thi qua cả dân chủ trực tiếp và dân chủ đại diện.",
        "Quyền lực phải được thể chế hóa bằng Hiến pháp, pháp luật và Nhà nước pháp quyền XHCN.",
      ],
    },
  },

  part3: {
    title: "Nhận định của nhóm: MXH có là bản chất dân chủ?",
    value: {
      title: "MXH là công cụ mạnh của dân chủ hiện đại",
      points: [
        "Thúc đẩy quyền làm chủ trực tiếp theo phương châm dân biết, dân bàn, dân làm, dân kiểm tra.",
        "Tăng hiệu quả giám sát và phản biện xã hội, buộc chính quyền lắng nghe và điều chỉnh.",
      ],
    },
    limit: {
      title: "Vì sao MXH chưa thể là bản chất dân chủ XHCN",
      points: [
        "Thiếu tính thể chế và pháp lý quyết định như các quy trình dân chủ theo luật.",
        "Dễ bị lợi dụng bởi thông tin sai lệch và thao túng dư luận.",
        "Vận hành theo lợi nhuận của nền tảng tư nhân nên không bảo đảm quyền làm chủ kinh tế - chính trị thực chất.",
      ],
    },
    conclusion:
      "Mạng xã hội là bước tiến về phương thức biểu hiện dân chủ trong thời đại số, nhưng chỉ là công cụ bổ trợ, không thể thay thế bản chất chính trị - kinh tế - pháp lý của dân chủ xã hội chủ nghĩa.",
  },

  part4: {
    title: "Kết luận",
    points: [
      "Khẳng định vai trò của công nghệ: MXH mở rộng không gian tham gia dân chủ nhanh và minh bạch hơn.",
      "Khẳng định bản chất không thể thay thế của dân chủ XHCN trên các mặt chính trị, kinh tế, pháp lý.",
      "Tại Việt Nam, sức mạnh dân chủ đến từ kết hợp dân chủ trực tiếp và dân chủ đại diện.",
      "Thông điệp cuối cùng: quyền làm chủ thực sự của nhân dân phải được thực hiện trong khuôn khổ pháp luật, dưới sự lãnh đạo của Đảng, vì mục tiêu xã hội công bằng, văn minh.",
    ],
  },
};
