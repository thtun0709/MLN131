export const heroContent = {
  badge: "CHUYÊN ĐỀ HỌC THUẬT",
  title: "DÂN CHỦ XÃ HỘI CHỦ NGHĨA",
  subtitle: "Giữa lý luận kinh điển và sức mạnh không gian mạng",
  statement:
    "Ngày nay, người dân có thể tự do bình luận, chia sẻ, tạo kiến nghị viral trên mạng xã hội và buộc chính quyền phải lắng nghe — đây chính là biểu hiện cao nhất của dân chủ xã hội chủ nghĩa mà Mác-Lênin đã đề ra",
  scrollHint: "Thông tin phân tích ở bên dưới",
  marxImageSrc: "",
  marxImageAlt: "Chân dung Karl Marx",
  typing: [
    "Mạng xã hội mở rộng phương thức tham gia dân chủ.",
    "Bản chất dân chủ xã hội chủ nghĩa được bảo đảm bằng thể chế và pháp luật.",
    "Công cụ kỹ thuật không thay thế nền tảng chính trị - kinh tế của chế độ.",
  ],
};

export const tabs = [
  { id: "chapter-1", label: "Chương 1" },
  { id: "chapter-2", label: "Chương 2" },
  { id: "chapter-3", label: "Chương 3" },
  { id: "chapter-4", label: "Chương 4" },
];

export const chapterData = {
  chapter1: {
    id: "chapter-1",
    label: "Chương 1",
    title: "Nhận định và phân tích tình huống",
    subtitle:
      "Nhận định ban đầu đúng một phần vì nhận ra vai trò mới của mạng xã hội, nhưng sai khi đồng nhất nó với biểu hiện cao nhất của dân chủ xã hội chủ nghĩa.",
    sections: [
      {
        heading: "Phần đúng của nhận định",
        subheading: "Mạng xã hội như một kênh dân chủ hiện đại",
        paragraphs: [
          "Mạng xã hội giúp lôi cuốn quần chúng tham gia quản lý xã hội nhanh và rộng hơn.",
          "Người dân có thêm công cụ ngôn luận, giám sát và phản biện chính sách công.",
        ],
        bullets: [
          "Mở rộng không gian Dân biết - Dân bàn - Dân làm - Dân kiểm tra.",
          "Tăng áp lực giải trình lên bộ máy chính quyền.",
          "Rút ngắn khoảng cách thông tin giữa người dân và cơ quan công quyền.",
        ],
        tone: "positive",
      },
      {
        heading: "Phần sai của nhận định",
        subheading: "Công cụ không đồng nghĩa với bản chất",
        paragraphs: [
          "Dân chủ xã hội chủ nghĩa là một chế độ chính trị - xã hội toàn diện, không chỉ là tiếp cận mạng.",
          "Nội dung lan truyền trên mạng xã hội chỉ là dư luận, không thay thế cơ chế quyền lực được luật định.",
        ],
        bullets: [
          "Nhầm lẫn giữa phương thức biểu hiện và bản chất chế độ.",
          "Thiếu góc nhìn giai cấp và vai trò lãnh đạo của Đảng.",
          "Không phản ánh nền tảng kinh tế: sở hữu xã hội về tư liệu sản xuất chủ yếu.",
        ],
        tone: "warning",
      },
    ],
  },
  chapter2: {
    id: "chapter-2",
    label: "Chương 2",
    title: "Căn cứ lý luận và bản chất dân chủ xã hội chủ nghĩa",
    subtitle:
      "Chương này tách thành các hợp phần lý luận độc lập để trình bày ngắn gọn, rõ cấu trúc và tránh gom nội dung vào một khối.",
    sections: [
      {
        heading: "Bản chất chính trị",
        subheading: "Quyền lực đại đa số dưới sự lãnh đạo của Đảng",
        paragraphs: [
          "Dân chủ xã hội chủ nghĩa mang bản chất giai cấp công nhân, đại diện lợi ích nhân dân lao động.",
        ],
        bullets: [
          "Đảng lãnh đạo để bảo đảm hướng phát triển đúng mục tiêu.",
          "Lôi cuốn quần chúng tham gia tự giác vào quản lý nhà nước và xã hội.",
          "Bảo vệ lợi ích chung trước các xu hướng dân túy và lợi dụng dân chủ.",
        ],
      },
      {
        heading: "Bản chất kinh tế",
        subheading: "Nền tảng vật chất của quyền làm chủ",
        paragraphs: [
          "Quyền làm chủ chính trị chỉ bền vững khi được bảo đảm bởi nền tảng kinh tế phù hợp.",
        ],
        bullets: [
          "Chế độ sở hữu xã hội về tư liệu sản xuất chủ yếu.",
          "Kết hợp quyền sở hữu - quản lý - phân phối vì người lao động.",
          "Tạo điều kiện thực chất để nhân dân tham gia quyết định các vấn đề lớn.",
        ],
      },
      {
        heading: "Bản chất tư tưởng - văn hóa",
        subheading: "Nền tảng giá trị khoa học và nhân văn",
        paragraphs: [
          "Hệ tư tưởng Mác - Lênin là cốt lõi, đồng thời kế thừa tinh hoa nhân loại.",
        ],
        bullets: [
          "Phát huy năng lực sáng tạo của cá nhân.",
          "Kết hợp hài hòa lợi ích cá nhân, tập thể và toàn xã hội.",
          "Hướng đến giải phóng con người và phát triển toàn diện.",
        ],
      },
      {
        heading: "Tiêu chí quyền lực thực sự thuộc về nhân dân",
        subheading: "Từ khẩu hiệu đến cơ chế vận hành",
        paragraphs: [
          "Quyền lực nhân dân phải được thể chế hóa trong Hiến pháp, pháp luật và cơ chế đại diện.",
        ],
        bullets: [
          "Dân chủ trực tiếp: góp ý, giám sát, tham gia quyết định.",
          "Dân chủ đại diện: Quốc hội, HĐND và cơ quan nhà nước do dân bầu.",
          "Nhà nước pháp quyền xã hội chủ nghĩa là công cụ bảo đảm quyền làm chủ bền vững.",
        ],
      },
    ],
  },
  chapter3: {
    id: "chapter-3",
    label: "Chương 3",
    title: "Nhận định của nhóm: Mạng xã hội có là bản chất dân chủ?",
    subtitle:
      "Thay vì trình bày gom, chương này chia thành các nhóm lập luận đối xứng để thấy rõ giá trị và giới hạn của mạng xã hội.",
    sections: [
      {
        heading: "Giá trị của mạng xã hội trong dân chủ hiện đại",
        subheading: "Công cụ hỗ trợ dân chủ trực tiếp",
        paragraphs: [
          "Mạng xã hội tạo không gian phản hồi nhanh, minh bạch và có tính kết nối cao.",
        ],
        bullets: [
          "Thúc đẩy tham gia xây dựng chính sách và giám sát xã hội.",
          "Buộc cơ quan công quyền tăng nghe, tăng phản hồi, tăng giải trình.",
          "Mở rộng hình thức dân chủ trực tiếp trong kỷ nguyên số.",
        ],
        tone: "positive",
      },
      {
        heading: "Giới hạn cốt lõi của mạng xã hội",
        subheading: "Không đủ điều kiện để đại diện bản chất chế độ",
        paragraphs: [
          "Tương tác trên nền tảng số không có giá trị pháp lý tương đương quy trình dân chủ theo luật.",
        ],
        bullets: [
          "Nguy cơ tin giả, thao túng dư luận, lợi dụng thông tin.",
          "Nền tảng thuộc sở hữu tư nhân, vận hành theo lợi nhuận.",
          "Không bảo đảm quyền làm chủ kinh tế - chính trị thực chất của đại đa số.",
        ],
        tone: "warning",
      },
      {
        heading: "Kết luận của nhóm",
        subheading: "Mạng xã hội là bổ trợ, không phải nền tảng bản chất",
        paragraphs: [
          "Mạng xã hội là bước tiến phương thức biểu hiện, giúp dân chủ xã hội chủ nghĩa sinh động hơn trong thực tiễn.",
        ],
        bullets: [
          "Cần kết hợp mạng xã hội với hệ thống thể chế - pháp lý chính thống.",
          "Bảo đảm quyền lực nhân dân được vận hành trong khuôn khổ nhà nước pháp quyền xã hội chủ nghĩa.",
        ],
        tone: "neutral",
      },
    ],
  },
  chapter4: {
    id: "chapter-4",
    label: "Chương 4",
    title: "Kết luận",
    subtitle:
      "Mỗi kết luận được đóng gói thành một thẻ card riêng để dễ nhìn, dễ trình chiếu và dễ thảo luận theo từng ý.",
    sections: [
      {
        heading: "Kết luận 1",
        subheading: "Không gian công nghệ là động lực phương thức",
        paragraphs: [
          "Mạng xã hội cần được nhìn nhận là bước tiến về phương thức thực hành dân chủ trong thời đại số.",
        ],
        bullets: [
          "Tăng tốc độ tiếp nhận ý kiến.",
          "Mở rộng khả năng giám sát xã hội.",
        ],
      },
      {
        heading: "Kết luận 2",
        subheading: "Bản chất dân chủ xã hội chủ nghĩa không thể thay thế",
        paragraphs: [
          "Cốt lõi vẫn là nền tảng chính trị - kinh tế - pháp lý của chế độ.",
        ],
        bullets: [
          "Chính trị: sự lãnh đạo của Đảng vì lợi ích nhân dân.",
          "Kinh tế: sở hữu xã hội về tư liệu sản xuất chủ yếu.",
          "Pháp lý: quyền lực nhân dân được bảo đảm bởi Hiến pháp và pháp luật.",
        ],
      },
      {
        heading: "Kết luận 3",
        subheading: "Thực tiễn Việt Nam",
        paragraphs: [
          "Sức mạnh dân chủ nằm ở sự vận hành đồng bộ giữa dân chủ trực tiếp và dân chủ đại diện.",
        ],
        bullets: [
          "Dân chủ trực tiếp qua giám sát, phản biện xã hội.",
          "Dân chủ đại diện qua Quốc hội, HĐND và cơ quan do dân bầu.",
        ],
      },
      {
        heading: "Thông điệp cuối cùng",
        subheading: "Quyền làm chủ đi kèm trách nhiệm công dân",
        paragraphs: [
          "Người dân cần thực hiện quyền làm chủ trong khuôn khổ pháp luật, tỉnh táo trước thông tin sai lệch.",
        ],
        bullets: [
          "Tránh bị cuốn vào xu hướng dân túy trên mạng.",
          "Hướng đến mục tiêu xã hội công bằng, văn minh.",
        ],
      },
    ],
  },
};
